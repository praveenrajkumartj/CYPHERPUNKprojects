...
export default function EventDetailCTA({ eventId, isRsvped, isBooked, isPaid, isLoggedIn, capacity, currentCount }: EventDetailCTAProps) {
  ...
  const [selectedTier, setSelectedTier] = useState<string | null>(isPaid ? 'Basic' : null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between px-2">
         ...
         <span className={`text-[10px] font-bold uppercase tracking-widest ${isSoldOut ? 'text-red-500' : 'text-emerald-400'}`}>
      ...
      <AnimatePresence>
        {!isPaid ? (
          <button
            ...
            className="w-full py-5 rounded-2xl bg-cyan-400 text-[#050510] font-black uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(34,211,238,0.3)] hover:bg-cyan-300 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? <Loader2 className="animate-spin w-5 h-5" /> : (
              isSoldOut ? 'SOLD OUT' : (isLoggedIn ? <><Zap size={18} /> RSVP NOW</> : 'SIGN IN TO RSVP')
            )}
          </button>
        ) : (
          <div className="space-y-4">
            ...
            <button
              onClick={() => selectedTier && handleBooking(selectedTier)}
              disabled={loading || !selectedTier || isSoldOut}
              className="w-full py-5 rounded-2xl bg-pink-500 text-white font-black uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(236,72,153,0.3)] hover:bg-pink-400 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? <Loader2 className="animate-spin w-5 h-5" /> : (
                isSoldOut ? 'SOLD OUT' : (isLoggedIn ? <><Ticket size={18} /> BOOK {selectedTier?.toUpperCase()} TICKET</> : 'SIGN IN TO BOOK')
              )}
            </button>
